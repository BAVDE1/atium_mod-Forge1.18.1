...
import com.BAVDE.atium_mod.AMStructures;
...
public class AtiumGeodeStructure extends StructureFeature<JigsawConfiguration> {
    public AtiumGeodeStructure(Codec<JigsawConfiguration> codec) {
        // Create the pieces layout of the structure and give it to the game
        super(codec, AtiumGeodeStructure::createPiecesGenerator, PostPlacementProcessor.f_192427_);
    }
    ...
    public static void setupStructureSpawns(final StructureSpawnListGatherEvent event) {
        if (event.getStructure() == AMStructures.ATIUM_GEODE_STRUCTURE.get()) {
    ...
    private static boolean isFeatureChunk(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        BlockPos blockPos = context.f_197355_().getWorldPosition();

        // Grab height of land. Will stop at first non-air block.
        int landHeight = context.f_197352_().getFirstOccupiedHeight(blockPos.getX(), blockPos.getZ(), Heightmap.Types.WORLD_SURFACE_WG, context.f_197357_());

        // Grabs column of blocks at given position. In overworld, this column will be made of stone, water, and air.
        // In nether, it will be netherrack, lava, and air. End will only be endstone and air. It depends on what block
        // the chunk generator will place for that dimension.
        NoiseColumn columnOfBlocks = context.f_197352_().getBaseColumn(blockPos.getX(), blockPos.getZ(), context.f_197357_());

        // Combine the column of blocks with land height and you get the top block itself which you can test.
        BlockState topBlock = columnOfBlocks.m_183556_(landHeight); //m_183556_ = blockstate (idk if this is right)
    ...
    public static Optional<PieceGenerator<JigsawConfiguration>> createPiecesGenerator(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        JigsawConfiguration newConfig = new JigsawConfiguration(
                ...
                () -> context.f_197360_().ownedRegistryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                        .get(new ResourceLocation(AtiumMod.MOD_ID, "atium_geode_structure/start_pool")),
        ...
        BlockPos blockpos = context.f_197355_().getMiddleBlockPosition(0);
